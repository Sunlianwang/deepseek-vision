...
1. **VS Code 用户**：不要直接粘贴图片，必须提供文件路径（VS Code 会把粘贴的图片作为 `image_url` 发给模型，纯文本模型不支持）。
2. **Codex / Claude Code 桌面端**：可以直接粘贴图片，MCP 工具会自动处理。
3. **不要**尝试自己"看"图片或"听"音频——你没有视觉/听觉能力，**必须**调用上述 MCP 工具。
4. **当用户在消息中看到文件路径**（如 `C:\Users\xxx\Pictures\Screenshots\xxx.png`）时，**必须调用 MCP 工具处理**，不要忽略它。
