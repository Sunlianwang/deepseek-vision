...
- **VS Code 用户**：不要直接粘贴图片，必须提供文件路径（VS Code 会把粘贴的图片作为 `image_url` 发给模型，纯文本模型不支持）。
- **Codex / Claude Code 桌面端**：可以直接粘贴图片，MCP 工具会自动处理。
- 媒体传**本地绝对路径**；音频暂不支持 URL；视频需本机安装 ffmpeg。
- **感知文本返回后，由你自己（主模型）基于它完成推理与回答**——本 MCP 只做眼睛，不做大脑。
- **当用户在消息中看到文件路径**（如 `C:\Users\xxx\Pictures\Screenshots\xxx.png`）时，**必须调用 MCP 工具处理**，不要忽略它。
